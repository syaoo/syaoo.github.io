...
计算时间序列的FFT以及其频谱（幅值谱，相位谱），功率谱、能量谱和倒频谱。
<!--more-->
FFT（Fast Fourier Transformation，离散傅里叶变换）是目前普遍使用的一种DFT的一种快速算法，其可将计算量从$n^2$降低到$n\log(n)$。当数据量很大时，使FFT的点数$n$为2的幂时，可明显提示计算效率。在Octave/Matlab中内置有相关函数可以方便的对数据做快速傅里叶变换处理；Python的Scipy、Numpy包中也提供有FFT计算相关的模块，PyFFW库也可以提供更快的FFT计算。本文以Octave程序为了，来探讨使用FFT来计算数据的频谱，功率谱等信息，在Python、Matlab中方法也类似。
...
这里`x_fft`是大小与`x`相同的复数序列，`x_fft(1)`与`x_fft(n/2+1)`分别表内直流分量（DC,$0\mathrm{Hz}$）和奈奎斯特频率分量（Nyquist 频率，$f_s/2\mathrm{Hz}$）。
...
把前面得到的幅值 *除*  **采样点数**修正幅值并绘制双边幅度谱图像，可以看到对应频点的幅值并不是真实幅值，这其实是因为FFT得到是幅值相同的正负频率，$[0,f_s/2]$和$[f_s/2,f_s]$分别对应$$[0,f_s/2]$$和$[-f_s/2,0]$，同`fftshift`可以将零频分量移动到频谱中心得到左图，所以真实的幅值还应在乘2。
...
![w3FlQ0.png](https://s1.ax1x.com/2020/09/09/w3FlQ0.png)
